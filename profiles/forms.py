...
def geocode_uk_postcode(postcode: str):
    ...
    if r.status_code == 200 and data.get("status") == 200:
        res = data["result"]
        return {
            "postcode": res["postcode"],  # normalized UK postcode format eg. N22 1AA
            "lat": res["latitude"],
            "lng": res["longitude"],
        }
    return None
...
class ProfileForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "bio": forms.Textarea(
                attrs={"rows": 4, "cols": 40, "placeholder": "Tell us something about yourself"}
            ),
    ...
    def clean_postal_code(self):
        ...
        # stash geodata and save to model in save()
        self._geo = geo
        return geo["postcode"]
