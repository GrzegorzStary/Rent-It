...
def items_view(request):
    ...
    if request.user.is_authenticated and max_km_param:
        ...
        if profile and profile.lat is not None and profile.lng is not None:
            ...
            lat_min, lat_max, lng_min, lng_max = bbox_around(
                user_lat, user_lng, max_km
            )
            # Prefilter fast by bounding box on owners' profile coordinates
            products = (
                products.select_related("user__profile").filter(
            ...
            messages.info(
                request,
                "Add a valid postcode to your profile "
                "to use the 'near me' filter.",
            )
    ...
    if radius_enabled and user_lat is not None and user_lng is not None:
        ...
        for p in product_list:
            ...
            if not prof or prof.lat is None or prof.lng is None:
                continue
            dist = haversine_km(
                user_lat, user_lng, float(prof.lat), float(prof.lng)
            )
            if dist <= max_km_val:
                p.distance_km = round(dist, 2)
                filtered.append(p)

        # Sort "near me" results by distance if no explicit sort chosen
...
def items_detail(request, pk):
    ...
    booked_ranges = Reservation.objects.filter(product=product).values(
        "start_date", "end_date"
    )
    ...
    context = {
        ...
        "is_hidden_owner_view": (
            not product.is_listed
            and request.user.is_authenticated
            and product.user == request.user
        ),
    }
...
def delete_image(request, image_id):
    image = get_object_or_404(
        ProductImage, id=image_id, product__user=request.user
    )
...
def toggle_listing(request, pk):
    ...
    if product.is_listed:
        messages.success(
            request, f'Listing enabled for "{product.name}".'
        )
    else:
        messages.warning(
            request, f'Listing disabled for "{product.name}".'
        )
...
def nearby_items(request):
    ...
    if not profile or profile.lat is None or profile.lng is None:
        messages.info(
            request,
            "Add a valid postcode to your profile to see nearby items.",
        )
        return redirect("items")
    ...
    for product in qs:
        ...
        if (
            owner_prof
            and owner_prof.lat is not None
            and owner_prof.lng is not None
        ):
            dist = haversine_km(
                user_lat,
                user_lng,
