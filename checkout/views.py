...
def _get_or_create_payment_intent(
    ...
    if pi_info:
        try:
            existing = stripe.PaymentIntent.retrieve(
                pi_info["id"]
            )
            if (
                existing
...
def checkout(request):
    ...
    if request.method == "GET":
        if "bag" in request.session:
            ...
            messages.error(
                request,
                "There's nothing in your bag to check out.",
            )
        ...
        if not checkout_data:
            messages.error(
                request,
                "There's nothing in your checkout at the moment.",
            )
            return redirect(reverse("items"))

        current_checkout = checkout_context(request)
        checkout_items = current_checkout.get("checkout_items", [])
        checkout_total = current_checkout.get(
            "checkout_total", Decimal("0.00")
        )
        deposit_total = current_checkout.get(
            "deposit_total", Decimal("0.00")
        )
        site_fee = (checkout_total * Decimal("0.10")).quantize(
            Decimal("0.01")
        )
        delivery = Decimal("0.00")
        grand_total = (
            checkout_total + deposit_total + site_fee + delivery
        ...
        if stripe_total < 30:
            messages.error(
                request,
                "Total must be at least £0.30 to proceed with payment.",
            )
        ...
        context = {
            ...
            "checkout_product_count": current_checkout.get(
    ...
    if request.method == "POST":
        ...
        form_data = {
            ...
            "delivery_cost": (
                request.POST.get("delivery_cost") or 0
            ),
        }
        ...
        if order_form.is_valid():
            ...
            checkout_total = current.get(
                "checkout_total", Decimal("0.00")
            )
            deposit_total = current.get(
                "deposit_total", Decimal("0.00")
            )
            site_fee = (checkout_total * Decimal("0.10")).quantize(
                Decimal("0.01")
            ...
            try:
                delivery_cost = Decimal(
                    str(form_data.get("delivery_cost") or 0)
                ).quantize(Decimal("0.01"))
            except Exception:
                delivery_cost = Decimal("0.00")

            grand_total = (
                checkout_total + deposit_total + site_fee + delivery_cost
            ).quantize(Decimal("0.01"))
            ...
            for item_id, item_data in checkout_data.items():
                try:
                    ...
                    if not raw_start or not raw_end:
                        messages.error(
                            request,
                            f"Rental dates for {product.name} are required.",
                        )
                        order.delete()
                    ...
                    try:
                        ...
                        messages.error(
                            request,
                            (
                                f"Invalid date format for {product.name}. "
                                "Use DD/MM/YYYY."
                            ),
                        )
                        order.delete()
                    ...
                    messages.error(
                        request,
                        (
                            "A product in your checkout wasn't found. "
                            "Please try again."
                        ),
                    )
                    order.delete()
...
def checkout_success(request, order_number):
        ...
        if owner and owner.email:
            owner_subject = render_to_string(
                (
                    ...
                    "owner_confirmation_email_subject.txt"
                ),
            ...
            owner_body = render_to_string(
                (
                    "checkout/confirmation_emails/"
                    "owner_confirmation_email_body.txt"
                ),
                {
                    "order": order,
                    "product": lineitem.product,
                    "renter": order.full_name,
                },
            )
    ...
    messages.success(
        ...
        (
            ...
            f"{order_number}. A confirmation email has been sent "
            f"to {order.email}."
        ),
    )
